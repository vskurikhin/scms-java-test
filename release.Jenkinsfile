pipeline {
    ...
    stages {
        ...
        stage('LiquibaseMigration') {
            steps {
                sh '''
                    /usr/local/liquibase/liquibase \
                        --classpath=./build/libs/scms-java-test-$(cat .VERSION).jar \
                        --changeLogFile=BOOT-INF/classes/db/changelog/db.changelog-master.yaml \
